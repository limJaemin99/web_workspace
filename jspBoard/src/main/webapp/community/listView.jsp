...
<div style="width:700px;margin: auto;padding: 10px;text-align: center;">
	...
	<c:forEach var="i" begin="${paging.startPage }" end="${paging.endPage }">
		...
		<!-- 클래스 이름 ieach 는 자바스크립트에서 숫자 a 태그 요소만 가져가기 위해 붙인 이름 -->
	</c:forEach>
...
<script type="text/javascript">
	...
	pnums.forEach(function(item){	/* forEach 로 숫자들을 하나씩 item 에 저장 */
		console.log(item);
		/* item 번호가 현재 페이지 이면 글꼴 스타일을 다르게함. */
		if(item.innerHTML=='${paging.currentPage}') {    /* a 태그의 숫자와 현재 페이지가 같은지 검사 */
